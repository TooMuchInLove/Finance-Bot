...
async def callback_account_info(query: CallbackQuery, state: FSMContext, account_service_selector: AccountServiceSelector) -> None:
    try:
        await query.answer()
...
@router.callback_query(ShowAccountCallbackData.filter(F.slug == ShowAccount.back), flags={"middlewares": ["DIMiddleware"]})
async def callback_account_back(query: CallbackQuery, state: FSMContext, account_service_changer: AccountServiceChanger) -> None:
    try:
        await query.answer()
...
@router.callback_query(ShowCategoryCallbackData.filter(F.slug == ShowCategory.get), flags={"middlewares": ["DIMiddleware"]})
async def callback_category_get(query: CallbackQuery, state: FSMContext, category_service_selector: CategoryServiceSelector) -> None:
    try:
        await query.answer()
        ...
        names_categories: list[tuple[str, str]] = [(category.name, category.name_detail) for category in categories if category]

        await send_telegram_message(
            message=query,
            message_text=response,
            is_update_text=True,
            buttons=create_buttons(create_categories_buttons(names=names_categories), create_account_back_button(), create_category_add_buttons()),
        )
...
async def callback_category_add(query: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        await send_telegram_message(message=query, message_text=response, is_delete_message=True)
    except Exception as error:
        logger.exception(error)


@router.callback_query(ShowCategoryCallbackData.filter(F.slug == ShowCategory.delete))
async def callback_category_delete(query: CallbackQuery, state: FSMContext, category_service_changer: CategoryServiceChanger) -> None:
    try:
        await query.answer()

        telegram_user_id = get_telegram_user_id(message=query)
        *_, name, name_detail = get_parameters(message=query.data)
        category = await category_service_changer.delete(telegram_user_id=telegram_user_id, name=name, name_detail=name_detail)
...
@router.callback_query(ShowWalletCallbackData.filter(F.slug == ShowWallet.get), flags={"middlewares": ["DIMiddleware"]})
async def callback_wallet_get(query: CallbackQuery, state: FSMContext, wallet_service_selector: WalletServiceSelector) -> None:
    try:
        await query.answer()
        ...
        await send_telegram_message(
            ...
            message_text=response,
            is_update_text=True,
            buttons=create_buttons(create_wallets_buttons(names_wallets), create_account_back_button(), create_wallet_add_buttons()),
        )
...
async def callback_wallet_delete(query: CallbackQuery, state: FSMContext, wallet_service_changer: WalletServiceChanger) -> None:
    try:
        await query.answer()

        telegram_user_id = get_telegram_user_id(message=query)
        *_, wallet_name = get_parameters(message=query.data)
        wallet = await wallet_service_changer.delete(telegram_user_id=telegram_user_id, name=wallet_name)
...
@router.callback_query(ShowWalletCallbackData.filter(F.slug == ShowWallet.get_detail), flags={"middlewares": ["DIMiddleware"]})
async def callback_wallet_get_detail(query: CallbackQuery, state: FSMContext, wallet_service_selector: WalletServiceSelector) -> None:
    try:
        await query.answer()
        ...
        *_, wallet_name = get_parameters(message=query.data)
        ...
        await send_telegram_message(
            ...
            message_text=response,
            is_update_text=True,
            buttons=create_buttons(create_wallet_get_back_button(), create_account_back_button()),
        )
